...
if __name__ == "__main__":
    ...
    objfun = optunity.cross_validated(x=data, y=labels, num_folds=5, regenerate_folds=True)(svm_rbf_tuned_auroc)
    objfun = negate(objfun)
    with open("pickle.pkl", 'w') as f: pickle.dump(objfun, f)

    #imports = ['optunity', 'optunity.metrics',
    #           'sklearn', 'sklearn.svm',
    #           'numpy as np', 'marshal', 'math', 'types',
    #           'sys', 'dill as pickle']

    search = {'logC': hp.uniform('logC', -4, 2), 'logGamma': hp.uniform('logGamma', -5, 0)}
    opt = hpolib_wrapper(objfun, search, budget, "TPE", "test")
    print(opt)
